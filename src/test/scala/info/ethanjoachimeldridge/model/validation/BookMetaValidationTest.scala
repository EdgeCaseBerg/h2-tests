...
class BookMetaValidationTest extends FlatSpec with Matchers{
	...
	it should "be invalid if the shortDescription is empty" in {
		val errors = Map("shortDescription" -> "Short description may not be empty")
		assertResult(errors) {
			BookMetaValidator(model.copy(
				bookId=1,title="NonEmpty",longDescription="NonEmpty"
			)).validate
		}	
	}

	it should "be invalid if the longDescription is empty" in {
		val errors = Map("longDescription" -> "Long description may not be empty")
		assertResult(errors) {
			BookMetaValidator(model.copy(
				bookId=1,title="NonEmpty",shortDescription="NonEmpty"
			)).validate
		}	
	}
